...
GAMES = ['Base_Game', 'Unicorns_of_Legend', 'Control_and_Chaos']
json_data = []

for game in GAMES:
    result = requests.get(BASE_URL + '/wiki/' + game)
    soup = BeautifulSoup(result.content, features="html.parser")
    cards_table = soup.find_all('table', class_='article-table')
    for table in cards_table:
        cards_list = table.find_all('tr')
